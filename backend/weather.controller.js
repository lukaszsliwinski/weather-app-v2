...
weather = (req, res) => {
  ...
  let weatherRequest = axios.get(
    `http://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${apiKey}`
  );
  let forecastRequest = axios.get(
    ...
    .then(
      axios.spread((weatherResponse, forecastResponse) => {
        ...
        for (i = 0; i < forecast.list.length; i++) {
          ...
          let dtPlusTimezone = forecast.list[i].dt + forecast.city.timezone;
          if (
            moment(nowPlusTimezone * 1000)
              .utc(false)
              .format('dddd') !=
              moment(dtPlusTimezone * 1000)
                .utc(false)
                .format('dddd') &&
            dtPlusTimezone % 86400 >= 0 &&
            dtPlusTimezone % 86400 <= 9900 // Check the rest of division - 0 is 0:00, 9900 is 2:45
          ) {
            try {
        ...
        res.json({
          ...
          weather: {
            ...
            sunrise: `${moment((weather.sys.sunrise + weather.timezone) * 1000)
              .utc(false)
              .format('HH:mm')}`,
            sunset: `${moment((weather.sys.sunset + weather.timezone) * 1000)
              .utc(false)
              .format('HH:mm')}`,
